...
For more help on a specific subcommand:
  pf <subcommand> --help
        ...
        self.parser.add_argument(
            '-f', '--file',
            help='Specify Pfyfile location (default: Pfyfile.pf)'
        )
        self.parser.add_argument(
            '--env',
            action='append',
        ...
        self.parser.add_argument(
            '--hosts',
            help='Comma-separated list of hosts (user@host:port)'
        )
        self.parser.add_argument(
            '--host',
            action='append',
            help='Single host (can be repeated)'
        )
        self.parser.add_argument(
            '--user',
            help='Default SSH user'
        )
        self.parser.add_argument(
            '--port',
        ...
        self.parser.add_argument(
            '--sudo-user',
            help='Run commands as specific sudo user'
        )
    ...
    def _add_builtin_commands(self):
        ...
        list_parser.add_argument(
            '--subcommand',
            help='Show tasks only from specific subcommand'
        )
    ...
    def parse_legacy_args(self, args: List[str]) -> Tuple[argparse.Namespace, List[str]]:
        ...
        while i < len(remaining_args):
            ...
            if '=' in arg and not arg.startswith('--'):
                ...
                if key in ('env', 'hosts', 'host', 'user', 'port', 'sudo', 'sudo_user', 'become', 'become_user'):
        ...
        if task_args and task_args[0] not in ('list', 'help', 'run'):
            ...
            if hasattr(self, '_subcommand_names') and potential_subcommand in self._subcommand_names:
                # It's a subcommand
...
Examples:
  ...
  pfuck --list                     # List all available tasks for reference
    ...
    parser.add_argument(
        '--file', '-f',
        help='Pfyfile to use for task discovery'
    )
    parser.add_argument(
        '--execute', '-e',
        action='store_true',
