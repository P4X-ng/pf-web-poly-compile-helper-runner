...
class PfyFile:
    ...
    name: str
    category: str
    tasks: List[Tuple[str, str]] = field(
...
class PfTUI:
            ...
            for pattern in patterns:
                for filepath in glob.glob(pattern):
                    ...
                    if basename == "Pfyfile.pf":
                        ...
                        category = (
                            basename[8:-3]
                            if basename.startswith("Pfyfile.")
                            else basename[:-3]
                        )
        ...
        for task_name, description, aliases in tasks_with_desc:
            ...
            if not categorized:
                if any(
                    ...
                    for keyword in ["exploit", "pwn", "rop", "shellcode", "gadget"]
                ):
                    ...
                    keyword in task_name.lower()
                    for keyword in ["practice", "demo", "vulnerable"]
                ):
    ...
    def show_header(self, subtitle_text: Optional[str] = None) -> None:
        ...
        if subtitle_text:
            ...
            subtitle = Text(
                "Use ↑↓ to navigate, Enter to select, q to quit/back", style="dim"
            )
    ...
    def _open_in_editor(self, filepath: str) -> None:
        ...
        if editor:
            ...
            if editor_name not in safe_editors and not (
                os.path.isabs(editor) and os.path.isfile(editor)
            ):
    ...
    def show_files_view(self) -> Optional[str]:
        ...
        while True:
            self.console.clear()
            self.show_header(
                "Browse Pfyfiles - Press e to edit, Enter to view tasks, q to go back"
            )
            ...
            for i, pfy_file in enumerate(
                self.pfy_files[start_idx:end_idx], start=start_idx
            ):
                task_count = len(pfy_file.tasks)
                prefix = "→ " if i == self.selected_index else "  "
                style = (
                    f"bold {pfy_file.color}"
                ...
                if i == self.selected_index:
                    self.console.print(
                        f"[{style}]{name_text}[/{style}] [dim]{task_info}[/dim]"
                    )
                else:
                    self.console.print(
                        f"[{style}]{name_text}[/{style}] [dim]{task_info}[/dim]"
            ...
            if end_idx < total_items:
                self.console.print(
                    f"    [dim]↓ {total_items - end_idx} more below[/dim]"
                )
            ...
            if key == "up" or key == "k":
                ...
                if self.selected_index < len(self.pfy_files) - 1:
                    ...
                    if (
                        self.selected_index
                        >= self.scroll_offset + self.max_display_items
                    ):
                        self.scroll_offset = (
                            self.selected_index - self.max_display_items + 1
                        )
            elif key == "\r" or key == "\n":
    ...
    def show_tasks_for_file(self) -> Optional[str]:
        ...
        while True:
            self.console.clear()
            self.show_header(
                f"Tasks in {self.selected_file.name} - Enter to run, q to go back"
            )
            ...
            for i, (task_name, description) in enumerate(
                tasks[start_idx:end_idx], start=start_idx
            ):
                prefix = "→ " if i == task_index else "  "
                style = (
                    f"bold {self.selected_file.color}" if i == task_index else "white"
                )

                desc_text = f" - {description}" if description else ""
                self.console.print(
                    f"[{style}]{prefix}{task_name}[/{style}][dim]{desc_text}[/dim]"
                )

            if end_idx < total_tasks:
                self.console.print(
                    f"    [dim]↓ {total_tasks - end_idx} more below[/dim]"
                )
    ...
    def list_tasks_by_category(self) -> None:
        ...
        for category in self.categories:
            for task_name, description in sorted(category.tasks):
                all_items.append(
                    (category.name, task_name, description, category.color)
                )
        ...
        while True:
            self.console.clear()
            self.show_header(
                "All Tasks - Enter to run, s for syntax check, q to go back"
            )
    ...
    def check_syntax(self) -> None:
        ...
        while True:
            self.console.clear()
            self.show_header(
                "Syntax Checker - Select task to check, Enter to check, q to go back"
            )
    ...
    def search_tasks(self) -> None:
        ...
        for category in self.categories:
            for task_name, description in category.tasks:
                if (
                    query_lower in task_name.lower()
                    or query_lower in (description or "").lower()
                ):
                    results.append(
                        (task_name, description, category.name, category.color)
                    )
        ...
        while True:
            self.console.clear()
            self.show_header(
                f"Search Results for '{query}' - Enter to run, s for syntax check, q to go back"
            )
            ...
            for i in range(start_idx, end_idx):
                ...
                self.console.print(
                    f"[{style}]{prefix}{task_name}[/{style}]{desc_text}{cat_text}"
                )
